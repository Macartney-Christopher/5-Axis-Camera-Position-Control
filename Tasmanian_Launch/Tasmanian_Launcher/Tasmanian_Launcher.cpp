...
#define MAXIMUM_FOCUS_STEP_SPEED 1000.0
#define MAXIMUM_ZOOM_STEP_SPEED 1000.0
...
void openSerial(LPCSTR port) {
	USES_CONVERSION;
	LPWSTR temp = A2W(port);

	if (portOpenFlag == false) {//If the port is not already open
		hSerial = CreateFile(temp, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);//Set the Com port to the correct arduino one
			...
			while (1) {
				...
				if (state.dwPacketNumber != lastDwPacketNumber) {
					...
					if (magnitude > INPUT_DEADZONE) { //check if the controller is outside a circular dead zone
						...
						RXShort = ((RX * scale) * abs(RX * scale) * maxPanStepSpeed);
						RYShort = ((RY * scale) * abs(RY * scale) * maxTiltStepSpeed);
					...
					if (magnitudeL > INPUT_DEADZONE) { //check if the controller is outside a circular dead zone
						...
						LXShort = (LX * scale) * abs(LX * scale) * maxSliderStepSpeed;
						LYShort = (LY * scale) * abs(LY * scale) * MAXIMUM_FOCUS_STEP_SPEED;
					}
					else {
						LXShort = 0;
						LYShort = 0;
					...
					short LTrig = -(state.Gamepad.bLeftTrigger);
					short RTrig = state.Gamepad.bRightTrigger;
					...
					if ((lastwButtons & RB_BUTTON) < (state.Gamepad.wButtons & RB_BUTTON)) {
						...
						sendCharArray((char*)";1"); //RB Execute moves
					}
